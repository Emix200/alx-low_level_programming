...
/**
 * check97 - check for the correct arg
...
/**
 * check98 - check the file
 * @check: check if T or F
 * @file: file_from name
 * @fd_from: file descriptor
 * @fd_to: file desc of file_to
 *
 * Return: NULL
 */
...
/**
 * check99 - check the file_to was created
 * @check: check if T of F
 * @file: file_to
 * @fd_from: file desc of file_from
 * @fd_to: file desc of file_to
 *
 * Return: NULL
 */
...
/**
 * check100 - check file were closed
 * @check: check iff true or false
 * @fd: file desc
 *
 * Return: NULL
 */
...
int main(int argc, char *argv[])
{
	...
	file_perm = S_IRUSR | S_IWUSR | S_IRGRP | SS_IWGRP | S_IROTH;
	...
	while (lenr == 1024)
	{
		...
		if (lenw != lenr)
			lenw = -1;
		check99(lenw, arg[2], fd_from, fd_to);
	}
